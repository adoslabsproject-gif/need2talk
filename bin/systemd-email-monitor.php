...
/**
 * SYSTEMD EMAIL SYSTEM MONITOR - ENTERPRISE MONITORING
 *
 * Sistema di monitoraggio avanzato per Systemd Email Workers
 * - Health check tramite systemctl
 * - Restart automatico via systemd
...
class SystemdEmailMonitor
{
    private $monitorId;
    private $checkInterval = 30; // 30 secondi
    private $logFile;
    private $maxWorkers;
    private $serviceName = 'need2talk-email-worker@';
    private $targetName = 'need2talk-email-workers.target';
    private $running = true; // Flag per controllo loop

    public function __construct($config = [])
    {
        $this->monitorId = 'systemd_monitor_'.date('YmdHis');
        $this->logFile = APP_ROOT.'/storage/logs/systemd-email-monitor.log';
        $this->maxWorkers = $config['max_workers'] ?? 2; // Default 2 workers (~800 emails capacity)
        $this->checkInterval = $config['check_interval'] ?? 30;

        $this->log('SYSTEMD EMAIL MONITOR STARTED', [
            'monitor_id' => $this->monitorId,
            'max_workers' => $this->maxWorkers,
            'check_interval' => $this->checkInterval,
            'target_service' => $this->targetName,
        ]);
    }

    public function startMonitoring(): void
    {
        $this->log('Starting continuous systemd monitoring...');

        while ($this->running) {
            try {
                $this->checkSystemdServiceHealth();
    ...
    {
        $results = [
            'services' => $this->checkSystemdServiceHealth(),
    ...
    private function checkSystemdServiceHealth(): array
    {
        $activeServices = [];
        $failedServices = [];
        $results = [];
        ...
        // Check target status
        $targetStatus = $this->executeSystemctl("is-active {$this->targetName}");
        $results['target_active'] = trim($targetStatus) === 'active';

        // Check individual worker services
        for ($i = 1; $i <= $this->maxWorkers; $i++) {
            $serviceName = "{$this->serviceName}{$i}.service";
            $status = $this->executeSystemctl("is-active $serviceName");
            ...
            if ($isActive) {
                ...
                $uptimeInfo = $this->executeSystemctl("show $serviceName --property=ActiveEnterTimestamp,NRestarts");
                $memoryInfo = $this->executeSystemctl("show $serviceName --property=MemoryCurrent");

                $results['services'][$i] = [
                    'status' => 'active',
                    'uptime_info' => trim($uptimeInfo),
                    'memory_info' => trim($memoryInfo),
                ];
            } else {
                $failedServices[] = $i;
                $results['services'][$i] = [
                    'status' => 'failed',
                    'error' => $this->executeSystemctl("is-failed $serviceName"),
                ];
            }
        }

        $this->log('Systemd service health check', [
            'active_count' => count($activeServices),
            'failed_count' => count($failedServices),
            'target_count' => $this->maxWorkers,
            'target_active' => $results['target_active'],
    ...
    private function checkEmailQueueBacklog(): array
    {
        ...
        try {
            ...
            $pending = $queue->getQueueSize();
            $processing = $queue->getProcessingSize();
            $failed = $queue->getFailedSize();
    ...
    private function restartFailedServices(): void
    {
        ...
        if (! empty($health['failed_services'])) {
            ...
            // Check if target needs restart
            if (! $health['target_active']) {
                $this->log('RESTARTING TARGET SERVICE', ['target' => $this->targetName]);
                $this->executeSystemctl("restart {$this->targetName}");
            }
        }
    }

    private function executeSystemctl(string $command): string
    {
        $fullCommand = "systemctl $command 2>&1";
    ...
    private function log(string $message, array $context = []): void
    {
        ...
        $logEntry = "[$timestamp] [SYSTEMD-MONITOR] $message$contextStr\n";
    ...
    private function logAlert(string $message, array $context = []): void
    {
        ...
        error_log("SYSTEMD EMAIL MONITOR ALERT: $message ".json_encode($context));
    }
...
if (function_exists('pcntl_signal')) {
    pcntl_signal(SIGTERM, function () use (&$monitorInstance) {
        echo "\nSystemd Email Monitor shutting down gracefully...\n";
        if ($monitorInstance) {
    ...
    pcntl_signal(SIGINT, function () use (&$monitorInstance) {
        echo "\nSystemd Email Monitor interrupted - shutting down...\n";
        if ($monitorInstance) {
            $monitorInstance->stop();
        }
        exit(0);
...
echo "🚀 SYSTEMD EMAIL SYSTEM MONITOR\n";
echo "===============================\n";
...
echo "Target Service: need2talk-email-workers.target\n";
echo "===============================\n\n";

try {
    $monitor = new SystemdEmailMonitor($config);
