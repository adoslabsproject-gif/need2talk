...
class ChatWidgetManager {
    ...
    #registerWebSocketHandlers() {
        // ENTERPRISE V10.36: Multiple fallback paths to find WebSocketManager
        // The object may be at different locations depending on script load order
        // ENTERPRISE V10.35: Check window.WebSocketManager first (most common)
        ...
        // Verify it has the .on() method we need
        if (typeof wsManager.on !== 'function') {
            // ENTERPRISE V11.8: Increased retry count and interval for slow connections
            // 5 retries × 1000ms = 5 seconds total wait time
            if (!this.#wsRetryCount) this.#wsRetryCount = 0;
            if (this.#wsRetryCount < 5) {
                this.#wsRetryCount++;
                console.warn(`[ChatWidget] WebSocketManager.on() not ready, retry ${this.#wsRetryCount}/5...`);
                setTimeout(() => this.#registerWebSocketHandlers(), 1000);
            } else {
                console.error('[ChatWidget] WebSocketManager found but missing .on() method after 5 retries!');
            }
            return;
        }
